...
namespace NetTopologySuite.Triangulate.QuadEdge
{
    ...
    public static class TrianglePredicate
    {
        ...
        private static void CheckRobustInCircle(Coordinate a, Coordinate b, Coordinate c,
            Coordinate p)
        {
            var nonRobustInCircle = IsInCircleNonRobust(a, b, c, p);
            var isInCircleDD = IsInCircleDDSlow(a, b, c, p);
            var isInCircleCC = IsInCircleCC(a, b, c, p);

            var circumCentre = Triangle.Circumcentre(a, b, c);
#if !PCL
// ReSharper disable RedundantStringFormatCall
            // String.Format needed to build 2.0 release!
            Debug.WriteLine(string.Format("p radius diff a = {0}",
                Math.Abs(p.Distance(circumCentre) - a.Distance(circumCentre))/a.Distance(circumCentre)));
            if ((nonRobustInCircle != isInCircleDD) || (nonRobustInCircle != isInCircleCC))
            {
                Debug.WriteLine(
                    string.Format(
                        "inCircle robustness failure (double result = {0}, DD result = {1}, CC result = {2})",
                        nonRobustInCircle, isInCircleDD, isInCircleCC));
                Debug.WriteLine(WKTWriter.ToLineString(new CoordinateArraySequence(new[] {a, b, c, p})));
                Debug.WriteLine(string.Format("Circumcentre = {0} radius = {1}", WKTWriter.ToPoint(circumCentre),
                    a.Distance(circumCentre)));
                Debug.WriteLine(string.Format("p radius diff a = {0}",
                    Math.Abs(p.Distance(circumCentre)/a.Distance(circumCentre) - 1)));
                Debug.WriteLine(string.Format("p radius diff b = {0}",
                    Math.Abs(p.Distance(circumCentre)/b.Distance(circumCentre) - 1)));
                Debug.WriteLine(string.Format("p radius diff c = {0}",

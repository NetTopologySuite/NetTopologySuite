...
namespace NetTopologySuite.Geography.Lib
{
    ...
    internal class GeodesicLine
    {
        ...
        private void LineInit(Geodesic g,
                              ...
                              int caps, (double first, double second) p)
        {
            _a = g._a;
            _f = g._f;
            _b = g._b;
            ...
            double cbet1, sbet1;
            GeoMath.sincosd(ref p, GeoMath.AngRound(_lat1));
            sbet1 = _f1 * p.first; cbet1 = p.second;
            // Ensure cbet1 = +epsilon at poles
            GeoMath.norm(ref p, sbet1, cbet1);
            ...
            GeoMath.norm(ref p, _ssig1, _csig1);
                                     ...
                                     int outmask)
        {
            ...
            if (arcmode)
            {
                ...
                r.a12 = s12_a12;
                sig12 = AngleUtility.ToRadians(s12_a12);
                (double first, double second) p = (0d, 0d);
                GeoMath.sincosd(ref p, s12_a12); ssig12 = p.first; csig12 = p.second;
            }
            else
            {
                ...
                r.a12 = AngleUtility.ToDegrees(sig12);
            }

            double sbet2, cbet2, salp2, calp2;
            // sig2 = sig1 + sig12
            ssig2 = _ssig1 * csig12 + _csig1 * ssig12;
            csig2 = _csig1 * csig12 - _ssig1 * ssig12;
            ...
            if ((outmask & GeodesicMask.LONGITUDE) != 0)
            {
                ...
                double lam12 = omg12 + _A3c *
                  (sig12 + (Geodesic.SinCosSeries(true, ssig2, csig2, _C3a)
                             - _B31));
                double lon12 = AngleUtility.ToDegrees(lam12);
        ...
        /// with<i> caps</i> |= <see cref="GeodesicMask.DISTANCE_IN"/>.
        /// </remarks>
        public void SetDistance(double s13)
        {
            _s13 = s13;
            var g = Position(false, _s13, 0);
            _a13 = g.a12;
        }
        ...
        {
            _a13 = a13;
            var g = Position(true, _a13, GeodesicMask.DISTANCE);
            _s13 = g.s12;
        }
         ...
         **********************************************************************/
        public (double first, double second) AzimuthCosines()
        {
            return Init() ? (_salp1, _calp1) : (double.NaN, double.NaN);
        }
         ...
         **********************************************************************/
        public (double first, double second) EquatorialAzimuthCosines()
        {
            return Init() ? (_salp0, _calp0) : (double.NaN, double.NaN);
        }

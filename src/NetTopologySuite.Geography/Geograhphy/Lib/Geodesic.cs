...
namespace NetTopologySuite.Geography.Lib
{
    ...
    internal class Geodesic
    {
        ...
        internal static readonly double tiny_ = Math.Sqrt(double.Epsilon);
        private static readonly double tol0_ = GeoMath.Ulp(1.0);
        // Increase multiplier in defn of tol1_ from 100 to 200 to fix inverse case
        // 52.784459512564 0 -52.784459512563990912 179.634407464943777557
        // which otherwise failed for Visual Studio 10 (Release and Debug)
        private static readonly double tol1_ = 200 * tol0_;
        private static readonly double tol2_ = Math.Sqrt(tol0_);
        ...
        public GeodesicLine GenDirectLine(double lat1, double lon1, double azi1,
                                          bool arcmode, double s12_a12, int caps)
        {
            azi1 = GeoMath.AngNormalize(azi1);
            double salp1, calp1;
            // Guard against underflow in salp0.  Also -0 is converted to +0.
            (double first, double second) p = (0d, 0d);
            GeoMath.sincosd(ref p, GeoMath.AngRound(azi1));
                                       ...
                                       double lat2, double lon2, int outmask)
        {
            ...
            double lon12, lon12s;
            GeoMath.AngDiff(ref p, lon1, lon2); lon12 = p.first; lon12s = p.second;
            ...
            double
              lam12 = AngleUtility.ToRadians(lon12), slam12, clam12;
            GeoMath.sincosd(ref p, lon12 > 90 ? lon12s : lon12);
            ...
            GeoMath.norm(ref p, sbet1, cbet1); sbet1 = p.first; cbet1 = p.second;
            cbet1 = Math.Max(tiny_, cbet1);

            GeoMath.sincosd(ref p, lat2);
            sbet2 = _f1 * p.first; cbet2 = p.second;
            // Ensure cbet2 = +epsilon at poles
            GeoMath.norm(ref p, sbet2, cbet2); sbet2 = p.first; cbet2 = p.second;
            ...
            if (meridian)
            {
                ...
                if (sig12 < 1 || m12x >= 0)
                {
                    ...
                    m12x *= _b;
                    s12x *= _b;
                    a12 = AngleUtility.ToDegrees(sig12);
                }
                else
                ...
                double dnm;
                {
                    var s =
                      InverseStart(sbet1, cbet1, dn1, sbet2, cbet2, dn2,
                                   ...
                                   C1a, C2a, p, v);
                    sig12 = s.sig12;
                    salp1 = s.salp1; calp1 = s.calp1;
                    salp2 = s.salp2; calp2 = s.calp2;
                    dnm = s.dnm;
                }
                ...
                else
                {
                    ...
                    for (bool tripn = false, tripb = false; numit < maxit2_; ++numit)
                    {
                        ...
                        Lambda12(w, sbet1, cbet1, dn1, sbet2, cbet2, dn2, salp1, calp1,
                                 slam12, clam12, numit < maxit1_, C1a, C2a, C3a, ref p, v);
                        V = w.lam12;
                        ...
                        if (numit < maxit1_ && dV > 0)
                        {
                            ...
                            if (nsalp1 > 0 && Math.Abs(dalp1) < Math.PI)
                            {
                                ...
                                salp1 = nsalp1;
                                GeoMath.norm(ref p, salp1, calp1);
                                salp1 = p.first; calp1 = p.second;
                        ...
                        GeoMath.norm(ref p, salp1, calp1); salp1 = p.first; calp1 = p.second;
                        tripn = false;
            ...
            if ((outmask & GeodesicMask.AREA) != 0)
            {
                ...
                if (calp0 != 0 && salp0 != 0)
                {
                    ...
                    GeoMath.norm(ref p, ssig1, csig1); ssig1 = p.first; csig1 = p.second;
                    GeoMath.norm(ref p, ssig2, csig2); ssig2 = p.first; csig2 = p.second;
                                    ...
                                    double lat2, double lon2, int outmask)
        {
            ...
            var result = InverseInt(lat1, lon1, lat2, lon2, outmask);
            var r = result.g;
        ...
        {
            var result = InverseInt(lat1, lon1, lat2, lon2, 0);
        ...
        /// <returns>
        /// A <see cref="GeodesicLine"/> object.
        /// </returns>
        /// <remarks>
        ...
        public static double SinCosSeries(bool sinp,
        ...
        {
            public double s12b, m12b, m0, M12, M21;
            public LengthsV()
            {
        ...
        private void Lengths(LengthsV v, double eps, double sig12,
        ...
        {
            public double sig12, salp1, calp1,
        ...
        private InverseStartV InverseStart(double sbet1, double cbet1, double dn1,
                                           ...
                                           // Scratch areas of the right size
                                           double[] C1a, double[] C2a,
                                           (double first, double second) p, LengthsV v)
        {
            ...
            if (shortline && ssig12 < _etol2)
            {
                ...
                GeoMath.norm(ref p, w.salp2, w.calp2);
            ...
            if (!(w.salp1 <= 0))
            {
                GeoMath.norm(ref p, w.salp1, w.calp1);
        ...
        {
            public double lam12, salp2, calp2, sig12, ssig1, csig1, ssig2, csig2,
              eps, domg12, dlam12;
            public Lambda12V()
            {
        ...
        private void Lambda12(Lambda12V w,
                              ...
                              ref (double first, double second) p, LengthsV v)
        {
            ...
            GeoMath.norm(ref p, w.ssig1, w.csig1);
            ...
            GeoMath.norm(ref p, w.ssig2, w.csig2);
        ...
        public double A3f(double eps)
        {
            // Evaluate A3
            return GeoMath.polyval(nA3_ - 1, _A3x, 0, eps);
        }

        public void C3f(double eps, double[] c)
        {
            // Evaluate C3 coeffs
        ...
        public void C4f(double eps, double[] c)
        {
            // Evaluate C4 coeffs

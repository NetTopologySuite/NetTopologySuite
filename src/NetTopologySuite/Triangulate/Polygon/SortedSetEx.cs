...
namespace System.Collections.Generic
{
    ...
    internal delegate bool TreeWalkPredicate<T>(SortedSetEx<T>.Node node);
...
#if !FEATURE_NETCORE
    [Serializable]
    public class SortedSetEx<T> : ISet<T>, ICollection<T>, ICollection, ISerializable, IDeserializationCallback, IReadOnlyCollection<T>
    {
#else
    public class SortedSet<T> : ISet<T>, ICollection<T>, ICollection, IReadOnlyCollection<T> {
            ...
            : this(comparer)
        {
            ...
            var baseSortedSet = collection as SortedSetEx<T>;
            ...
            if (baseSortedSet != null && baseTreeSubSet == null && AreComparersEqual(this, baseSortedSet))
            {
                ...
                var theirStack = new Stack<SortedSetEx<T>.Node>(2 * log2(baseSortedSet.Count) + 2);
                var myStack = new Stack<SortedSetEx<T>.Node>(2 * log2(baseSortedSet.Count) + 2);
                var theirCurrent = baseSortedSet.root;
                var myCurrent = (theirCurrent != null ? new SortedSetEx<T>.Node(theirCurrent.Item, theirCurrent.IsRed) : null);
                root = myCurrent;
                while (theirCurrent != null)
                {
                    theirStack.Push(theirCurrent);
                    myStack.Push(myCurrent);
                    myCurrent.Left = (theirCurrent.Left != null ? new SortedSetEx<T>.Node(theirCurrent.Left.Item, theirCurrent.Left.IsRed) : null);
                ...
                while (theirStack.Count != 0)
                {
                    ...
                    if (theirRight != null)
                    {
                        myRight = new SortedSetEx<T>.Node(theirRight.Item, theirRight.IsRed);
                    }
                    ...
                    while (theirRight != null)
                    {
                        ...
                        myRight.Left = (theirRight.Left != null ? new SortedSetEx<T>.Node(theirRight.Left.Item, theirRight.Left.IsRed) : null);
        ...
        internal virtual bool InOrderTreeWalk(TreeWalkPredicate<T> action, bool reverse)
        {
            ...
            var stack = new Stack<Node>(2 * (int)(SortedSetEx<T>.log2(Count + 1)));
            var current = root;
            while (current != null)
            {
                stack.Push(current);
        ...
        internal virtual bool BreadthFirstTreeWalk(TreeWalkPredicate<T> action)
        {
            ...
            var processQueue = new List<Node>();
        ...
        public void UnionWith(IEnumerable<T> other)
        {
            ...
            {
                var dummy = new SortedSetEx<T>(s, this.comparer);
            ...
            { //this actually hurts if N is much greater than M the /2 is arbitrary
                ...
                root = SortedSetEx<T>.ConstructRootFromSortedArray(merged, 0, c - 1, null);
                count = c;
                version++;
            }
            else
        ...
        {
            //
            var toSave = new List<T>(this.Count);
        ...
        public void SymmetricExceptWith(IEnumerable<T> other)
        {
            ...
            else
            {
                ...
                var elements = (new List<T>(other)).ToArray();
        ...
        public T Min
        {
            ...
            {
                var ret = default(T);
                InOrderTreeWalk(delegate (SortedSetEx<T>.Node n) { ret = n.Item; return false; });
                return ret;
            }
        }
        ...
        public T Max
        {
            ...
            {
                var ret = default(T);
                InOrderTreeWalk(delegate (SortedSetEx<T>.Node n) { ret = n.Item; return false; }, true);
                return ret;
            }
        }
...
#if !FEATURE_NETCORE
        [Serializable]
        internal sealed class TreeSubSet : SortedSetEx<T>, ISerializable, IDeserializationCallback
        {
#else
        internal sealed class TreeSubSet : SortedSet<T> {
#endif
            SortedSetEx<T> underlying;
            T min, max;
            ...
            internal override bool InOrderTreeWalk(TreeWalkPredicate<T> action, bool reverse)
            {
                ...
                // The maximum height of a red-black tree is 2*lg(n+1).
                // See page 264 of "Introduction to algorithms" by Thomas H. Cormen
                var stack = new Stack<Node>(2 * (int)SortedSetEx<T>.log2(count + 1)); //this is not exactly right if count is out of date, but the stack can grow
                var current = root;
                while (current != null)
                {
            ...
            internal override SortedSetEx<T>.Node FindNode(T item)
            {
            ...
            public override SortedSetEx<T> GetViewBetween(T lowerValue, T upperValue)
            {
                ...
                var ret = (TreeSubSet)underlying.GetViewBetween(lowerValue, upperValue);
                return ret;
            }
        ...
        internal class Node
        {
            ...
            public Node Left { get => _left; set { _left = value; if (value != null) value.Parent = this; } }
            public Node Right { get => _right; set { _right = value; if (value != null) value.Parent = this; } }
            public Node Parent;
        ...
        public struct Enumerator : IEnumerator<T>, IEnumerator {
#endif
            private SortedSetEx<T> tree;
            private int version;


            private Stack<SortedSetEx<T>.Node> stack;
            private SortedSetEx<T>.Node current;
            static SortedSetEx<T>.Node dummyNode = new SortedSetEx<T>.Node(default(T));
            ...
            internal Enumerator(SortedSetEx<T> set)
            {
                ...
                stack = new Stack<SortedSetEx<T>.Node>(2 * (int)SortedSetEx<T>.log2(set.Count + 1));
                current = null;
                reverse = false;
#if !FEATURE_NETCORE
                siInfo = null;
            ...
            private void GetObjectData(SerializationInfo info, StreamingContext context)
            {
                ...
                info.AddValue(TreeName, tree, typeof(SortedSetEx<T>));
            ...
            private void OnDeserialization(object sender)
            {
                ...
                tree = (SortedSetEx<T>)siInfo.GetValue(TreeName, typeof(SortedSetEx<T>));
                ...
                stack = new Stack<SortedSetEx<T>.Node>(2 * (int)SortedSetEx<T>.log2(tree.Count + 1));
                current = null;
                if (EnumStarted)
                {
                    var item = (T)siInfo.GetValue(NodeValueName, typeof(T));
                    Intialize();
        ...
        public T Above(T key)
        {
            return LowestGreaterThan(key, false, false).Item;
        }

        public T Below(T key)
        {
            return HighestLessThan(key, false).Item;
        }

        private Node HighestLessThan(T key, bool equal)
        {
            var last = LastNode();
            var current = Root;
            int comparison = 0;
            ...
            {
                last = current;
                comparison = comparer.Compare(key, current.Item);
                if (comparison > 0)
                    current = current.Right;
        ...
        private Node LowestGreaterThan(T key, bool first, bool equal)
        {
            var last = first ? FirstNode() : null;
            var current = Root;
    ...
    /// </summary>    
    internal class SortedSetEqualityComparer<T> : IEqualityComparer<SortedSetEx<T>>
    {
        ...
        public bool Equals(SortedSetEx<T> x, SortedSetEx<T> y)
        {
            return SortedSetEx<T>.SortedSetEquals(x, y, comparer);
        }

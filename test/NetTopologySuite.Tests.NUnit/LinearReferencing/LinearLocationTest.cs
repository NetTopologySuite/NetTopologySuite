...
namespace NetTopologySuite.Tests.NUnit.LinearReferencing
{
    ...
    public class LinearLocationTest
    {
        ...
        public void TestEqualsHashCodeConsistentWithNaN()
        {
            ...
            Assert.That(zero.Equals(nan), Is.False);
            Assert.That(nan.Equals(half), Is.False);
            Assert.That(zero.Equals(half), Is.False);
            Assert.That(nan, Is.EqualTo(new LinearLocation(0, 0, double.NaN)));
            Assert.That(nan.GetHashCode(), Is.EqualTo(new LinearLocation(0, 0, double.NaN).GetHashCode()));
        }

...
namespace NetTopologySuite.Geometries
{
    ...
    /// capturing the topological relationship between two <see cref="Geometry"/>s. 
    ...
    /// <para>
    /// The entries of the matrix are defined by the constants in the <see cref="Dimension"/>s enum.
    ...
    public class IntersectionMatrix
    {
        ...
        {                      
            if (requiredDimensionSymbol == '*')
                return true;            
            if (requiredDimensionSymbol == 'T' && (actualDimensionValue >= Dimension.Point || actualDimensionValue == Dimension.True))
                return true;            
            if (requiredDimensionSymbol == 'F' && actualDimensionValue == Dimension.False)
                return true;            
            if (requiredDimensionSymbol == '0' && actualDimensionValue == Dimension.Point)
                return true;            
            if (requiredDimensionSymbol == '1' && actualDimensionValue == Dimension.Curve)
                return true;
            if (requiredDimensionSymbol == '2' && actualDimensionValue == Dimension.Surface)
                return true;
            return false;
        }
        ...
        public bool IsCrosses(Dimension dimensionOfGeometryA, Dimension dimensionOfGeometryB)
        {
            if ((dimensionOfGeometryA == Dimension.Point   && dimensionOfGeometryB == Dimension.Curve)   ||
                (dimensionOfGeometryA == Dimension.Point   && dimensionOfGeometryB == Dimension.Surface) ||
                (dimensionOfGeometryA == Dimension.Curve   && dimensionOfGeometryB == Dimension.Surface))            
                return Matches(_matrix[(int) Location.Interior, (int) Location.Interior], 'T') &&
                       Matches(_matrix[(int) Location.Interior, (int) Location.Exterior], 'T');
            ...
            if ((dimensionOfGeometryA == Dimension.Curve   && dimensionOfGeometryB == Dimension.Point)   ||
                ...
                return Matches(_matrix[(int) Location.Interior, (int) Location.Interior], 'T') &&
                       Matches(_matrix[(int) Location.Exterior, (int) Location.Interior], 'T');

...
namespace NetTopologySuite.Tests.NUnit.Simplify
{
    ...
    {
        private static readonly IWktGeometryReader<Coordinate> Reader = GeometryUtils.GeometryFactory.WktReader;

        public static IGeometry<Coordinate>[] GetResult(String wkt, Double tolerance)
        {
            IGeometry<Coordinate>[] ioGeom = new IGeometry<Coordinate>[2];
            ioGeom[0] = Reader.Read(wkt);
            ioGeom[1] = DouglasPeuckerSimplifier<Coordinate>.Simplify(ioGeom[0], tolerance);

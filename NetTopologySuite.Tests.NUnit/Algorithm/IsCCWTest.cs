...
namespace NetTopologySuite.Tests.NUnit.Algorithm
{
    ...
    public class IsCCWTest
    {
        ...
        public void TestCCW()
        {
            ICoordinate[] pts = GetCoordinates("POLYGON ((60 180, 140 240, 140 240, 140 240, 200 180, 120 120, 60 180))");
            Assert.AreEqual(CGAlgorithms.IsCCW(pts), false);

            ICoordinate[] pts2 = GetCoordinates("POLYGON ((60 180, 140 120, 100 180, 140 240, 60 180))");
            Assert.AreEqual(CGAlgorithms.IsCCW(pts2), true);
            // same pts list with duplicate top point - check that isCCW still works
            ICoordinate[] pts2x = GetCoordinates("POLYGON ((60 180, 140 120, 100 180, 140 240, 140 240, 60 180))");

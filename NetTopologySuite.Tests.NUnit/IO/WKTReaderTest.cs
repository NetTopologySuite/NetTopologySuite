...
namespace NetTopologySuite.Tests.NUnit.IO
{
    ...
    public class WKTReaderTest : GeometryTestCase
    {
        ...
        public void TestReadNaN()
        {
            ...
            var pt2 = (Point)this.reader2DOld.Read("POINT (10 10 nan)");
            var pt3 = (Point)this.reader2DOld.Read("POINT (10 10 NAN)");
        ...
        public void TestReadPoint()
        {
            ...
            // act
            var pt2D = (Point)this.reader2D.Read("POINT (10 10)");
            var pt2DE = (Point)this.reader2D.Read("Point EMPTY");
            var pt3D = (Point)this.reader3D.Read("POINT Z(10 10 10)");
            var pt2DM = (Point)this.reader2DM.Read("POINT M(10 10 11)");
            var pt3DM = (Point)this.reader3DM.Read("POINT ZM(10 10 10 11)");
        ...
        public void TestReadLineString()
        {
            ...
            // act
            var ls2D = (LineString)this.reader2D.Read("LINESTRING (10 10, 20 20, 30 40)");
            var ls2DE = (LineString)this.reader2D.Read("LineString EMPTY");
            var ls3D = (LineString)this.reader3D.Read("LINESTRING Z(10 10 10, 20 20 10, 30 40 10)");
            var ls2DM = (LineString)this.reader2DM.Read("LINESTRING M(10 10 11, 20 20 11, 30 40 11)");
            var ls3DM = (LineString)this.reader3DM.Read("LINESTRING ZM(10 10 10 11, 20 20 10 11, 30 40 10 11)");
        ...
        public void TestReadLinearRing()
        {
            ...
            // act
            var ls2D = (LineString)this.reader2D.Read("LINEARRING (10 10, 20 20, 30 40, 10 10)");
            var ls2DE = (LineString)this.reader2D.Read("LinearRing EMPTY");
            var ls3D = (LineString)this.reader3D.Read("LINEARRING Z(10 10 10, 20 20 10, 30 40 10, 10 10 10)");
            var ls2DM = (LineString)this.reader2DM.Read("LINEARRING M(10 10 11, 20 20 11, 30 40 11, 10 10 11)");
            var ls3DM = (LineString)this.reader3DM.Read("LINEARRING ZM(10 10 10 11, 20 20 10 11, 30 40 10 11, 10 10 10 11)");
        ...
        public void TestReadMultiPoint()
        {
            ...
            // act
            var rdr = this.reader2D;
            var mP2D = (MultiPoint)rdr.Read("MULTIPOINT ((10 10), (20 20))");
            var mP2DE = (MultiPoint)rdr.Read("MultiPoint EMPTY");
            rdr = this.reader3D;
            var mP3D = (MultiPoint)rdr.Read("MULTIPOINT Z((10 10 10), (20 20 10))");
            rdr = this.reader2DM;
            var mP2DM = (MultiPoint)rdr.Read("MULTIPOINT M((10 10 11), (20 20 11))");
            rdr = this.reader3DM;
            var mP3DM = (MultiPoint)rdr.Read("MULTIPOINT ZM((10 10 10 11), (20 20 10 11))");

            // assert
            Assert.That(CheckEqual(csMP2D[0], ((Point)mP2D.GetGeometryN(0)).CoordinateSequence));
            Assert.That(CheckEqual(csMP2D[1], ((Point)mP2D.GetGeometryN(1)).CoordinateSequence));
            Assert.That(mP2DE.IsEmpty);
            Assert.That(mP2DE.NumGeometries, Is.Zero);
            Assert.That(CheckEqual(csMP3D[0], ((Point)mP3D.GetGeometryN(0)).CoordinateSequence));
            Assert.That(CheckEqual(csMP3D[1], ((Point)mP3D.GetGeometryN(1)).CoordinateSequence));
            Assert.That(CheckEqual(csMP2DM[0], ((Point)mP2DM.GetGeometryN(0)).CoordinateSequence));
            Assert.That(CheckEqual(csMP2DM[1], ((Point)mP2DM.GetGeometryN(1)).CoordinateSequence));
            Assert.That(CheckEqual(csMP3DM[0], ((Point)mP3DM.GetGeometryN(0)).CoordinateSequence));
            Assert.That(CheckEqual(csMP3DM[1], ((Point)mP3DM.GetGeometryN(1)).CoordinateSequence));
        }
        ...
        public void TestReadMultiLineString()
        {
            ...
            // act
            var rdr = this.reader2D;
            var mLs2D = (MultiLineString)rdr.Read("MULTILINESTRING ((10 10, 20 20), (15 15, 30 15))");
            var mLs2DE = (MultiLineString)rdr.Read("MultiLineString EMPTY");
            rdr = this.reader3D;
            var mLs3D = (MultiLineString)rdr.Read("MULTILINESTRING Z((10 10 10, 20 20 10), (15 15 10, 30 15 10))");
            rdr = this.reader2DM;
            var mLs2DM = (MultiLineString)rdr.Read("MULTILINESTRING M((10 10 11, 20 20 11), (15 15 11, 30 15 11))");
            rdr = this.reader3DM;
            var mLs3DM = (MultiLineString)rdr.Read("MULTILINESTRING ZM((10 10 10 11, 20 20 10 11), (15 15 10 11, 30 15 10 11))");

            // assert
            Assert.That(CheckEqual(csMls2D[0], ((LineString)mLs2D.GetGeometryN(0)).CoordinateSequence));
            Assert.That(CheckEqual(csMls2D[1], ((LineString)mLs2D.GetGeometryN(1)).CoordinateSequence));
            Assert.That(mLs2DE.IsEmpty);
            Assert.That(mLs2DE.NumGeometries, Is.Zero);
            Assert.That(CheckEqual(csMls3D[0], ((LineString)mLs3D.GetGeometryN(0)).CoordinateSequence));
            Assert.That(CheckEqual(csMls3D[1], ((LineString)mLs3D.GetGeometryN(1)).CoordinateSequence));
            Assert.That(CheckEqual(csMls2DM[0], ((LineString)mLs2DM.GetGeometryN(0)).CoordinateSequence));
            Assert.That(CheckEqual(csMls2DM[1], ((LineString)mLs2DM.GetGeometryN(1)).CoordinateSequence));
            Assert.That(CheckEqual(csMls3DM[0], ((LineString)mLs3DM.GetGeometryN(0)).CoordinateSequence));
            Assert.That(CheckEqual(csMls3DM[1], ((LineString)mLs3DM.GetGeometryN(1)).CoordinateSequence));
        }
        ...
        public void TestReadMultiPolygon()
        {
            ...
            var poly2DE = (MultiPolygon)rdr.Read("MultiPolygon EMPTY");
            rdr = this.reader3D;
            MultiPolygon[] poly3D =
            {
            ...
            Assert.That(CheckEqual(csPoly2D[2], ((Polygon)poly2D[2].GetGeometryN(1)).ExteriorRing.CoordinateSequence));
            ...
            Assert.That(CheckEqual(csPoly3D[2], ((Polygon)poly3D[2].GetGeometryN(1)).ExteriorRing.CoordinateSequence));

            Assert.That(CheckEqual(csPoly2DM[0], ((Polygon)poly2DM[2].GetGeometryN(0)).ExteriorRing.CoordinateSequence));
            Assert.That(CheckEqual(csPoly2DM[1], ((Polygon)poly2DM[2].GetGeometryN(0)).GetInteriorRingN(0).CoordinateSequence));
            Assert.That(CheckEqual(csPoly2DM[2], ((Polygon)poly2DM[2].GetGeometryN(1)).ExteriorRing.CoordinateSequence));
            ...
            Assert.That(CheckEqual(csPoly3DM[2], ((Polygon)poly3DM[2].GetGeometryN(1)).ExteriorRing.CoordinateSequence));
        }
        ...
        public void TestReadGeometryCollection()
        {
            ...
            var gc0 = (GeometryCollection)rdr.Read("GEOMETRYCOLLECTION (POINT (10 10), POINT (30 30), LINESTRING (15 15, 20 20))");
            var gc1 = (GeometryCollection)rdr.Read("GEOMETRYCOLLECTION (POINT (10 10), LINEARRING EMPTY, LINESTRING (15 15, 20 20))");
            var gc2 = (GeometryCollection)rdr.Read("GEOMETRYCOLLECTION (POINT (10 10), LINEARRING (10 10, 20 20, 30 40, 10 10), LINESTRING (15 15, 20 20))");

...
namespace NetTopologySuite.Tests.NUnit.Noding
{
    ...
    public class SegmentPointComparatorFullTest
    {
        ...
        {
            var seg0 = new Coordinate(0, 0);
            var seg1 = new Coordinate(x, y);
            var seg = new LineSegment(seg0, seg1);
            for (var i = 0; i < 4; i++)
            {
                double dist = i;
                var gridSize = 1 / pm.Scale;
        ...
        {
            var p0 = computePoint(seg, dist0);
            var p1 = computePoint(seg, dist1);
            if (p0.Equals(p1))
            {
        ...
        private void CheckNodePosition(LineSegment seg, Coordinate p0, Coordinate p1, int expectedPositionValue)
        {
            ...
            var posValue = SegmentPointComparator.Compare(octant, p0, p1);

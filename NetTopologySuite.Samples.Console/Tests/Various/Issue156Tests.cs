...
namespace NetTopologySuite.Samples.Tests.Various
{
    ...
    public class Issue156Tests
    {
        ...
        public void TestTransform()
        {
            ...
            var sourceCs = csFactory.CreateFromWkt(sourceCsWkt);
            ...
            var targetCs = csFactory.CreateFromWkt(targetCsWkt);
            Assert.That(targetCs, Is.Not.Null);
            var ctFactory = new CoordinateTransformationFactory();
            var coordTransformation = ctFactory.CreateFromCoordinateSystems(sourceCs, targetCs);
            ...
            var mp = (IMultiPoint)geom;
            foreach (IPoint pt in mp.Geometries)
            {
                var tp = GeometryTransform.TransformGeometry(gf, pt, coordTransformation.MathTransform);
                Assert.That(tp, Is.Not.Null);
                Assert.That(tp.IsValid, Is.True);
            }
            var transformed = GeometryTransform.TransformGeometry(gf, mp, coordTransformation.MathTransform);

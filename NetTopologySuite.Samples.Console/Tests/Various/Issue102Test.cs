...
namespace NetTopologySuite.Tests.Various
{
    public class Issue102Test
    {
        ...
        public void TestRun()
        {
            ...
            try
            {
                // We create the directory if this one doesn't exist
                var path = Path.Combine(Path.GetTempPath(), "TaggedLineSimplifier-MT");
                ...
                const int max = 100;
                var index = 0;
                var pathPattern = Path.Combine(path, "SimplifyCoordinates{0}.txt");
                var simplifyReferencePathPattern = Path.Combine(path, "SIMPLIFY_REFERENCE.txt");
                var notSimplifyReferencePathPattern = Path.Combine(path,"NOT_SIMPLIFY_REFERENCE.txt");
                bool more;

                // In this first case, we write the coordinates of the line
                Write(notSimplifyReferencePathPattern, line.Coordinates);
                // In this second case, we run only one simplify function and write the result (coordinates)
                var res = (ILineString)TopologyPreservingSimplifier.Simplify(line, tolerance);
                Simplify(simplifyReferencePathPattern, line, tolerance, res);
                // In this second, we run the simplify function in multithreading context and write the result (coordinates)
                Init();
                for (index = 0; index < max; index++)
                {
                    Task.Factory.StartNew(() => Simplify(pathPattern, line, tolerance, res), TaskCreationOptions.LongRunning);
                }
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="pathPattern">Complete of the file where writes</param>
        ...
        public void Simplify(string pathPattern, ILineString line, double tolerance, ILineString supposedResult)
        {
            try
            {
                var geometry = TopologyPreservingSimplifier.Simplify((ILineString)line.Clone(), tolerance);
                Write(pathPattern, geometry.Coordinates);
                Assert.AreEqual(line.CoordinateSequence.Count, supposedResult.CoordinateSequence.Count);
                Assert.IsTrue(line.Equals(supposedResult));
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
        ...
        public void Write(string pathPattern, IEnumerable<Coordinate> listCoords)
        {
            try
            {
                using (TextWriter tw = new StreamWriter(string.Format(pathPattern, GetFileIndex())))
                {
                    tw.WriteLine("X;Y");
                    foreach (var coord in listCoords)
                    {
                        tw.WriteLine(coord.X + ";" + coord.Y);
                    }
                }
            }
            catch (Exception ex)
